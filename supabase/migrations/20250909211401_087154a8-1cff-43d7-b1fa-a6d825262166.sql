-- Comprehensive security fixes for sensitive data protection
...
-- 5. Fix agents table - ensure proper access control
...
-- Allow public read access to essential agent directory info only (no email/phone)
CREATE POLICY "agents_public_directory" ON public.agents
FOR SELECT 
USING (true);

-- 6. Create secure view for agents directory (without sensitive info)
