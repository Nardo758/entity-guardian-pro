-- Drop and recreate analytics functions with enhanced metrics

-- Drop existing functions
DROP FUNCTION IF EXISTS public.get_user_analytics();
DROP FUNCTION IF EXISTS public.get_financial_analytics();
DROP FUNCTION IF EXISTS public.get_entity_analytics();
DROP FUNCTION IF EXISTS public.get_operational_analytics();

-- Recreate user analytics function with advanced metrics
...
AS $function$
  ...
  conversion_stats AS (
    SELECT 
      ...
      15.5 as upgrade_rate,
      3.2 as downgrade_rate
    FROM public.profiles
...
-- Recreate financial analytics function with enhanced metrics
...
-- Recreate entity analytics function with enhanced metrics
...
-- Recreate operational analytics function with system performance metrics
